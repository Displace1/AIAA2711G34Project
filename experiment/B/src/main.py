...
# Common bin edges for consistent comparison
bins = np.linspace(0, 12, 61)   # 60 bins from 0 to 12

# Direct sampling
N_hist = 10000
x_dir_hist = sample_p(N_hist)
axes1[0].hist(x_dir_hist, bins=bins, density=True, alpha=0.6, color='blue', label='Direct MC')
...
axes1[0].set_xlim(0, 12)          # <-- 统一横坐标范围
axes1[0].set_ylim(0, 0.5)         # 可选：统一纵坐标，便于比较
axes1[0].legend()

# Rejection sampling
x_rej_hist, _ = rejection_sampling(N_hist)
axes1[1].hist(x_rej_hist, bins=bins, density=True, alpha=0.6, color='green', label='Rejection Sampling')
...
axes1[2].hist(x_is_hist, bins=bins, weights=w_hist, density=True, alpha=0.6, color='red', label='IS weighted')
...
axes1[2].set_xlim(0, 12)
axes1[2].set_ylim(0, 0.5)
axes1[2].legend()
